...
export default function HealthStatus() {
  ...
  const fetchData = async (date: Date) => {
    ...
    try {
      const stepRes = await axios.get(`http://192.168.0.55:8000/api/fitdata/by-date/?date=${dateStr}`, {
    ...
    try {
      const bpRes = await axios.get(`http://192.168.0.55:8000/api/healthcare/by-date/?date=${dateStr}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setBpData({
