...
export default function MedicationInfoScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get('http://172.20.10.3:8000/api/mednames/', {
        headers: {
  ...
  const handleDelete = async (prescriptionID: string) => {
    try {
      ...
      await axios.delete(
        `http://172.20.10.3:8000/api/delete-prescription/${prescriptionID}/`,
        {
  ...
  const handleTakePhoto = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://172.20.10.3:8000/ocr-analyze/',
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
