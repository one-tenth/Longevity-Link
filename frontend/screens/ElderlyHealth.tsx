...
export default function ElderlyHealth() {
  ...
  const uploadStepsToBackend = async (steps: number, timestamp: Date) => {
    ...
    try {
      const response = await axios.post('http://192.168.1.84:8000/api/fitdata/', {
        steps,
  ...
  const fetchBloodPressure = async (date: Date) => {
    ...
    try {
      const response = await axios.get(`http://172.20.10.2:8000/api/healthcare/by-date/?date=${dateStr}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
